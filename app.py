...
<style>
    ...
    :root {
        --primary: #4f46e5; --primary-hover: #4338ca; --primary-light: #e0e7ff; --primary-text: #3730a3;
        --secondary: #7c3aed; --secondary-hover: #6d28d9; --secondary-light: #ede9fe; --secondary-text: #5b21b6;
        --accent: #0891b2; --accent-light: #cffafe; --accent-text: #155e75;
        --success: #059669; --success-light: #d1fae5; --success-text: #065f46;
        --warning: #d97706; --warning-light: #fef3c7; --warning-text: #92400e;
        --error: #dc2626; --error-light: #fee2e2; --error-text: #991b1b;
        --info: #2563eb; --info-light: #dbeafe; --info-text: #1e40af;
        --bg-primary: #f8fafc; --bg-secondary: #ffffff; --bg-tertiary: #e2e8f0;
        --bg-card: #ffffff; --bg-hover: #f1f5f9; --bg-selected: #e0e7ff;
        --text-primary: #0f172a; --text-secondary: #334155; --text-tertiary: #475569;
        --text-muted: #64748b; --text-inverse: #ffffff;
        --border-color: #cbd5e1; --border-hover: #94a3b8; --border-focus: var(--primary);
    ...
    [data-theme="dark"] {
        ...
        --bg-card: #1e293b; --bg-hover: #334155; --bg-selected: #312e81;
        --text-primary: #f8fafc; --text-secondary: #e2e8f0; --text-tertiary: #cbd5e1;
        --text-muted: #94a3b8; --text-inverse: #0f172a;
        --border-color: #475569; --border-hover: #64748b;
        --primary-light: #312e81; --secondary-light: #4c1d95; --accent-light: #164e63;
        --success-light: #064e3b; --warning-light: #78350f; --error-light: #7f1d1d; --info-light: #1e3a8a;
    ...
    * { box-sizing: border-box; }
    ...
    body {
        ...
        background-image: radial-gradient(at 40% 20%, rgba(79, 70, 229, 0.06) 0px, transparent 50%),
    ...
    [data-testid="stSidebar"] {
        ...
        border-right: 2px solid var(--border-color); box-shadow: var(--shadow-lg);
    }
    ...
    .main-header {
        text-align: center; padding: 2.5rem 2rem; margin: 1rem 0 2rem 0;
    ...
    .main-header::before {
        ...
        animation: pulse 10s ease-in-out infinite; pointer-events: none;
    }
    @keyframes pulse { 0%, 100% { opacity: 0.3; transform: scale(1); } 50% { opacity: 0.5; transform: scale(1.08); } }
    .main-header h1 {
        font-weight: 800 !important; font-size: 2.8rem !important; color: white !important;
        margin: 0 !important; text-shadow: 0 2px 10px rgba(0, 0, 0, 0.2);
        position: relative; z-index: 2; letter-spacing: -0.02em;
    }
    .main-header .subtitle {
        font-size: 1.1rem; color: rgba(255, 255, 255, 0.95); margin: 1rem 0 0 0;
        line-height: 1.7; position: relative; z-index: 2; max-width: 900px;
        margin-left: auto; margin-right: auto; font-weight: 400;
    }
    .main-header .badge {
        display: inline-flex; align-items: center; gap: 6px; padding: 6px 16px;
        background: rgba(255, 255, 255, 0.2); color: white; border-radius: var(--radius-full);
        font-size: 0.85rem; font-weight: 600; margin-top: 1rem;
        border: 1px solid rgba(255, 255, 255, 0.3);
    }

    .metric-card {
        background: var(--bg-card); border-radius: var(--radius-lg); padding: 20px 18px;
        border: 2px solid var(--border-color); box-shadow: var(--shadow-md);
        transition: var(--transition-normal); position: relative; overflow: hidden; height: 100%;
    ...
    .metric-card:hover { transform: translateY(-3px); box-shadow: var(--shadow-xl); border-color: var(--primary); }
    .metric-card .metric-icon { font-size: 1.6rem; margin-bottom: 6px; display: block; }
    .metric-card .metric-label {
        font-size: 0.8rem; color: var(--text-tertiary); font-weight: 700;
    ...
    .metric-delta.positive { background: var(--success-light); color: var(--success-text); border: 1px solid var(--success); }
    .metric-delta.negative { background: var(--error-light); color: var(--error-text); border: 1px solid var(--error); }
    .metric-delta.neutral { background: var(--bg-tertiary); color: var(--text-tertiary); border: 1px solid var(--border-color); }
    .metric-delta.warning { background: var(--warning-light); color: var(--warning-text); border: 1px solid var(--warning); }

    .section-card {
        background: var(--bg-card); border-radius: var(--radius-lg); padding: 24px;
        margin-bottom: 20px; box-shadow: var(--shadow-md); border: 2px solid var(--border-color);
        transition: var(--transition-normal);
    }
    .section-card:hover { box-shadow: var(--shadow-lg); border-color: var(--primary); }
    .section-card h3 {
        font-weight: 700; color: var(--text-primary); margin: 0 0 16px 0;
    ...
    .section-card h3 .icon {
        ...
        border-radius: var(--radius-md); color: var(--primary-text); font-weight: 700;
    }

    .stButton>button {
        background: linear-gradient(135deg, var(--primary), var(--primary-hover));
        color: white !important; border-radius: var(--radius-md); padding: 12px 24px;
        font-weight: 700; border: 2px solid transparent; transition: var(--transition-normal);
    ...
    [data-testid="stDataFrame"] th {
        background: linear-gradient(135deg, var(--primary), var(--secondary));
        color: white !important; font-weight: 700; padding: 14px 16px;
        text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.6px;
    ...
    [data-testid="stDataFrame"] tr:hover { background: var(--bg-hover); }
    [data-testid="stDataFrame"] tr:last-child td { border-bottom: none; }

    .status-badge {
        display: inline-flex; align-items: center; gap: 5px; padding: 5px 12px;
        border-radius: var(--radius-full); font-size: 0.75rem; font-weight: 700;
        text-transform: uppercase; letter-spacing: 0.5px; white-space: nowrap; border: 1px solid;
    }
    ...
    .stTabs [data-baseweb="tab-list"] {
        gap: 6px; background: var(--bg-tertiary); padding: 5px;
        border-radius: var(--radius-lg); border: 2px solid var(--border-color);
    }
    .stTabs [data-baseweb="tab"] {
        border-radius: var(--radius-md); padding: 10px 20px; font-weight: 700;
        transition: var(--transition-fast); color: var(--text-tertiary);
        font-size: 0.9rem; border: 2px solid transparent !important;
    }
    .stTabs [data-baseweb="tab"]:hover {
        background: var(--bg-hover); color: var(--text-primary); border-color: var(--border-hover) !important;
    }
    .stTabs [aria-selected="true"] {
        background: linear-gradient(135deg, var(--primary), var(--secondary));
        color: white !important; box-shadow: var(--shadow-md);
        transform: translateY(-1px); border-color: transparent !important;
    }

    .insight-card {
        background: var(--bg-card); border-radius: var(--radius-lg); padding: 18px 20px;
        margin-bottom: 14px; border-left: 5px solid var(--primary);
        box-shadow: var(--shadow-sm); border: 2px solid var(--border-color);
        transition: var(--transition-normal); display: flex; align-items: flex-start; gap: 12px;
    }
    .insight-card:hover { box-shadow: var(--shadow-md); transform: translateX(3px); border-color: var(--primary); }
    .insight-card .insight-icon { font-size: 1.4rem; flex-shrink: 0; margin-top: 2px; }
    .insight-card .insight-content { flex: 1; }
    .insight-card .insight-title {
        font-weight: 700; font-size: 1rem; color: var(--text-primary);
        margin-bottom: 5px; display: flex; align-items: center; gap: 6px;
    }
    .insight-card .insight-message {
        color: var(--text-secondary); line-height: 1.6; font-size: 0.9rem; font-weight: 500;
    }
    .insight-card.warning { border-left-color: var(--warning); background: linear-gradient(135deg, rgba(217, 119, 6, 0.05), transparent); }
    .insight-card.success { border-left-color: var(--success); background: linear-gradient(135deg, rgba(5, 150, 105, 0.05), transparent); }
    .insight-card.error { border-left-color: var(--error); background: linear-gradient(135deg, rgba(220, 38, 38, 0.05), transparent); }
    .insight-card.info { border-left-color: var(--info); background: linear-gradient(135deg, rgba(37, 99, 235, 0.05), transparent); }

    .match-status-card {
        background: var(--bg-card); border-radius: var(--radius-lg); padding: 18px;
        border: 2px solid var(--border-color); border-left: 5px solid var(--primary);
        box-shadow: var(--shadow-sm); transition: var(--transition-normal); margin-bottom: 12px;
    }
    ...
    .match-status-card .status-name { font-weight: 700; font-size: 1rem; color: var(--text-primary); }
    .match-status-card .status-count {
        background: var(--bg-tertiary); padding: 4px 12px; border-radius: var(--radius-full);
        font-weight: 800; font-size: 0.9rem; color: var(--text-primary); border: 1px solid var(--border-color);
    }
    .match-status-card .status-details { font-size: 0.85rem; color: var(--text-secondary); line-height: 1.5; font-weight: 500; }
    .match-status-card .status-link {
        display: inline-flex; align-items: center; gap: 4px; color: var(--primary);
        font-weight: 700; font-size: 0.85rem; margin-top: 8px; text-decoration: none; transition: var(--transition-fast);
    }
    .match-status-card .status-link:hover { color: var(--primary-hover); text-decoration: underline; }

    .party-card {
        background: var(--bg-card); border-radius: var(--radius-md); padding: 14px;
        border: 2px solid var(--border-color); margin: 6px 0; transition: var(--transition-fast);
        display: flex; align-items: center; justify-content: space-between; gap: 14px;
    }
    .party-card:hover { border-color: var(--primary); box-shadow: var(--shadow-sm); background: var(--bg-hover); }
    .party-info { flex: 1; }
    .party-name { font-weight: 700; color: var(--text-primary); margin-bottom: 3px; font-size: 0.95rem; }
    .party-gstin { font-size: 0.75rem; color: var(--text-tertiary); font-family: 'JetBrains Mono', monospace; font-weight: 500; }
    .party-stats { text-align: right; }
    .party-value { font-weight: 800; color: var(--text-primary); font-size: 1.05rem; }
    .party-label { font-size: 0.75rem; color: var(--text-tertiary); font-weight: 600; text-transform: uppercase; letter-spacing: 0.4px; }

    .theme-toggle { position: fixed; bottom: 20px; right: 20px; z-index: 1000; }
    .theme-toggle button {
        background: var(--bg-card); border: 2px solid var(--border-color);
        border-radius: var(--radius-full); width: 46px; height: 46px;
        display: flex; align-items: center; justify-content: center; font-size: 1.2rem;
        cursor: pointer; box-shadow: var(--shadow-lg); transition: var(--transition-normal);
        color: var(--text-primary); font-weight: 700;
    }
    .theme-toggle button:hover {
        transform: scale(1.1) rotate(15deg); border-color: var(--primary);
        background: var(--primary-light); color: var(--primary-text);
    }

    .footer {
        text-align: center; padding: 36px 24px; margin-top: 50px;
        background: linear-gradient(135deg, var(--bg-card), var(--bg-tertiary));
        border-radius: var(--radius-2xl) var(--radius-2xl) 0 0;
        border-top: 2px solid var(--border-color); position: relative;
    }
    .footer::before {
        content: ''; position: absolute; top: 0; left: 50%; transform: translateX(-50%);
        width: 80%; height: 2px; background: linear-gradient(90deg, transparent, var(--border-color), transparent);
    }
    .footer .brand {
    ...
    .footer .credits { color: var(--text-tertiary); font-size: 0.9rem; margin: 3px 0; font-weight: 500; }
    .footer .version {
        display: inline-block; background: var(--bg-tertiary); padding: 5px 16px;
        border-radius: var(--radius-full); font-size: 0.8rem; color: var(--text-tertiary);
        margin-top: 14px; border: 1px solid var(--border-color); font-weight: 600;
    }
    .footer a {
    ...
    .footer a:hover { color: var(--primary); background: var(--primary-light); }

    .quick-actions { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 14px; margin: 20px 0; }
    .quick-action-btn {
        background: var(--bg-card); border: 2px solid var(--border-color);
        border-radius: var(--radius-lg); padding: 18px 14px; text-align: center;
        cursor: pointer; transition: var(--transition-normal); text-decoration: none;
    ...
    .quick-action-btn .icon { font-size: 1.8rem; display: block; transition: var(--transition-fast); }
    .quick-action-btn:hover .icon { transform: scale(1.1); }
    .quick-action-btn .label { font-weight: 700; font-size: 0.85rem; }

    .doc-type-badge {
        display: inline-flex; align-items: center; padding: 4px 10px;
        border-radius: var(--radius-full); font-size: 0.75rem; font-weight: 700;
        margin: 2px; text-transform: uppercase; letter-spacing: 0.4px; border: 1px solid;
    }
    .doc-type-invoice { background: var(--success-light); color: var(--success-text); border-color: var(--success); }
    .doc-type-credit { background: var(--error-light); color: var(--error-text); border-color: var(--error); }
    .doc-type-debit { background: var(--warning-light); color: var(--warning-text); border-color: var(--warning); }
    ...
    .animate-fade-in { animation: fadeInUp 0.45s ease forwards; opacity: 0; }
    ...
    .toast {
        position: fixed; bottom: 20px; right: 20px; padding: 13px 20px;
        border-radius: var(--radius-lg); box-shadow: var(--shadow-xl); z-index: 10000;
        animation: slideIn 0.28s ease; max-width: 400px; display: flex;
        align-items: center; gap: 10px; border: 2px solid var(--border-color); font-weight: 600;
    }
    .toast.success { background: var(--bg-card); border-left: 5px solid var(--success); color: var(--text-primary); }
    .toast.error { background: var(--bg-card); border-left: 5px solid var(--error); color: var(--text-primary); }
    .toast.warning { background: var(--bg-card); border-left: 5px solid var(--warning); color: var(--text-primary); }
    @keyframes slideIn { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }

    .stTextInput > div > div > input, .stNumberInput > div > div > input, .stSelectbox > div > div > div {
        border-radius: var(--radius-md); border: 2px solid var(--border-color);
        transition: var(--transition-fast); background: var(--bg-secondary);
        color: var(--text-primary); font-weight: 500; padding: 10px 14px;
    }
    .stTextInput > div > div > input:focus, .stNumberInput > div > div > input:focus, .stSelectbox > div > div > div:focus {
        border-color: var(--primary); box-shadow: var(--focus-ring); outline: none;
    }
    .stCheckbox label, .stRadio label { font-weight: 600; color: var(--text-secondary); font-size: 0.95rem; }
    .stSlider > div > div > div { background: var(--primary) !important; }
    .streamlit-expanderHeader {
        background: var(--bg-tertiary); border-radius: var(--radius-md);
        border: 2px solid var(--border-color); font-weight: 700; padding: 11px 15px;
        transition: var(--transition-fast); font-size: 0.95rem;
    }
    .streamlit-expanderHeader:hover { background: var(--bg-hover); border-color: var(--primary); }
    .chart-container {
        background: var(--bg-card); border-radius: var(--radius-lg); padding: 18px;
        border: 2px solid var(--border-color); box-shadow: var(--shadow-sm); margin: 14px 0;
    }
    .status-legend {
        display: flex; flex-wrap: wrap; gap: 10px; padding: 14px;
        background: var(--bg-tertiary); border-radius: var(--radius-md);
        margin: 14px 0; border: 1px solid var(--border-color);
    }
    .legend-item { display: flex; align-items: center; gap: 5px; font-size: 0.8rem; color: var(--text-secondary); font-weight: 500; }
    .legend-dot { width: 11px; height: 11px; border-radius: var(--radius-full); display: inline-block; border: 1px solid var(--border-color); }
    .formula-box {
        background: var(--bg-tertiary); border-radius: var(--radius-md); padding: 10px 14px;
        font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; color: var(--text-secondary);
        margin: 6px 0; border-left: 3px solid var(--accent); overflow-x: auto;
        border: 1px solid var(--border-color); font-weight: 500;
    }

    @media (max-width: 1200px) { .main-header h1 { font-size: 2.3rem !important; } .metric-card .metric-value { font-size: 1.7rem; } }
    @media (max-width: 992px) {
        .main-header { padding: 2rem 1.5rem; } .main-header h1 { font-size: 2rem !important; }
        .main-header .subtitle { font-size: 1rem; } .metric-card { padding: 18px 16px; }
        .metric-card .metric-value { font-size: 1.6rem; } .section-card { padding: 20px; }
        .quick-actions { grid-template-columns: repeat(2, 1fr); }
    }
    @media (max-width: 768px) {
        .main-header h1 { font-size: 1.8rem !important; } .metric-card .metric-value { font-size: 1.5rem; }
        .quick-actions { grid-template-columns: 1fr; } .section-card { padding: 18px; }
        .footer { padding: 28px 16px; } .stTabs [data-baseweb="tab"] { padding: 9px 16px; font-size: 0.85rem; }
    }
    @media (max-width: 480px) {
        .main-header h1 { font-size: 1.6rem !important; } .metric-card .metric-value { font-size: 1.4rem; }
        .party-card { flex-direction: column; text-align: center; } .party-stats { text-align: center; }
    }
    ...
    @media print {
        ...
        body { background: white !important; } .metric-card, .insight-card { break-inside: avoid; }
    }

    *:focus-visible { outline: 2px solid var(--primary); outline-offset: 2px; }

    @media (prefers-contrast: high) {
        :root { --border-color: #000; --text-secondary: #000; --text-tertiary: #333; }
        [data-theme="dark"] { --border-color: #fff; --text-secondary: #fff; --text-tertiary: #ddd; }
    }
</style>
...
function toggleTheme() {
    ...
    if (window.Streamlit && window.Streamlit.setComponentValue) { window.Streamlit.setComponentValue(newTheme); }
}
...
def parse_date(date_str: str) -> Optional[datetime]:
    ...
    formats = ['%d-%m-%Y', '%Y-%m-%d', '%d/%m/%Y', '%m/%d/%Y', '%d-%b-%Y', '%d %b %Y', '%b %d, %Y', '%Y/%m/%d', '%d.%m.%Y', '%m.%d.%Y']
    for fmt in formats:
        try:
...
def generate_sample_2b_template() -> bytes:
    ...
    cols = ["SUPPLIER GSTIN", "DOCUMENT NUMBER", "TAXABLE VALUE", "IGST", "CGST", "SGST", 
            "SUPPLIER NAME", "MY GSTIN", "DOCUMENT DATE", "MONTH", "DOC_TYPE", "REVERSE_CHARGE", "SECTION_NAME"]
    sample_data = [
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        header_format = workbook.add_format({"bold": True, "bg_color": "#1e40af", "font_color": "white", "border": 1, "align": "center", "valign": "vcenter"})
        for col_num, col_name in enumerate(cols):
            worksheet.write(0, col_num, col_name, header_format)
        worksheet.set_column('A:A', 20); worksheet.set_column('B:B', 22); worksheet.set_column('C:F', 14)
        worksheet.set_column('G:G', 35); worksheet.set_column('H:H', 20); worksheet.set_column('I:I', 14)
        worksheet.set_column('J:J', 14); worksheet.set_column('K:M', 14)
...
def generate_sample_books_template() -> bytes:
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        worksheet.set_column('A:A', 20); worksheet.set_column('B:B', 22); worksheet.set_column('C:F', 14)
        worksheet.set_column('G:G', 35); worksheet.set_column('H:H', 20); worksheet.set_column('I:I', 14)
        worksheet.set_column('J:J', 14); worksheet.set_column('K:O', 14)
        worksheet.data_validation('K2:K1000', {'validate': 'list', 'source': ['INVOICE', 'CREDIT', 'DEBIT']})
    return output.getvalue()
...
with col_upload3:
    ...
    with col_d1:
        st.download_button(label="📥 2B Sample", data=generate_sample_2b_template(), file_name="GSTR2B_Sample_Template.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="btn_download_2b_sample", help="Download sample GSTR-2B template")
    with col_d2:
        st.download_button(label="📘 PR Sample", data=generate_sample_books_template(), file_name="PurchaseRegister_Sample.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="btn_download_pr_sample", help="Download sample Purchase Register")
...
def process_reconciliation(file_2b_bytes: bytes, file_pr_bytes: bytes, tolerance: float, date_tol_days: int, include_rc: bool, handle_cdn_neg: bool, fuzzy_threshold: float, validate_gstin_flag: bool, strict_fy: bool) -> Tuple[pd.DataFrame, int, pd.DataFrame, pd.DataFrame, Dict]:
    ...
    try:
        ...
        col_map = {'SUPPLIER GSTIN': 'SUPPLIER_GSTIN', 'DOCUMENT NUMBER': 'DOC_NUMBER', 'TAXABLE VALUE': 'TAXABLE_VALUE', 'SUPPLIER NAME': 'SUPPLIER_NAME', 'MY GSTIN': 'MY_GSTIN', 'DOCUMENT DATE': 'DOC_DATE', 'DOC_TYPE': 'DOC_TYPE', 'REVERSE_CHARGE': 'REVERSE_CHARGE', 'ITC_CLAIM_TYPE': 'ITC_CLAIM_TYPE', 'PLACE_OF_SUPPLY': 'PLACE_OF_SUPPLY', 'MONTH': 'MONTH', 'SECTION_NAME': 'SECTION_NAME'}
        for old, new in col_map.items():
            if old in df_2b.columns: df_2b[new] = df_2b[old]
            if old in df_pr.columns: df_pr[new] = df_pr[old]
        
        # Ensure required columns exist
        required = ['SUPPLIER_GSTIN', 'DOC_NUMBER', 'TAXABLE_VALUE', 'SUPPLIER_NAME', 'MY_GSTIN', 'DOC_DATE', 'IGST', 'CGST', 'SGST']
        for col in required:
            if col not in df_2b.columns: df_2b[col] = None
            if col not in df_pr.columns: df_pr[col] = None
        for df in [df_2b, df_pr]:
            if 'CESS' not in df.columns: df['CESS'] = 0
            if 'SECTION_NAME' not in df.columns: df['SECTION_NAME'] = 'B2B'
        ...
        for df in [df_2b, df_pr]:
            ...
            if 'DOC_TYPE' not in df.columns or df['DOC_TYPE'].isna().any():
                ...
                df['DOC_TYPE'] = df['DOC_TYPE'].replace({'CREDIT NOTE': 'CREDIT', 'DEBIT NOTE': 'DEBIT', 'CDN': 'CREDIT', 'CN': 'CREDIT', 'CR': 'CREDIT', 'DBN': 'DEBIT', 'DN': 'DEBIT', 'DB': 'DEBIT', 'INV': 'INVOICE', 'B2B': 'INVOICE', 'B2C': 'INVOICE', 'I': 'INVOICE'})
        ...
        # KEY CHANGE: Tax difference is now 2B - PR (signed value, not absolute)
        merged['TAX_DIFF'] = merged['TOTAL_TAX_2B'].fillna(0) - merged['TOTAL_TAX_PR'].fillna(0)
        ...
        merged['DATE_DIFF_DAYS'] = merged.apply(lambda r: calculate_date_difference(r['DOC_DATE_2B'], r['DOC_DATE_PR']), axis=1)
        ...
        if fuzzy_threshold < 100:
            merged['NAME_FUZZY_MATCH'] = merged.apply(lambda r: fuzzy_match_names(r['SUPPLIER_NAME_2B'], r['SUPPLIER_NAME_PR'], fuzzy_threshold), axis=1)
        else:
...
def create_enhanced_excel_export(merged_df: pd.DataFrame, df_2b: pd.DataFrame, df_pr: pd.DataFrame, stats: Dict, include_charts: bool = True, include_raw_data: bool = True, add_dropdown: bool = True, include_subtotals: bool = False) -> bytes:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Define formats
        header_format = workbook.add_format({'bold': True, 'bg_color': '#1e40af', 'font_color': 'white', 'border': 1, 'align': 'center', 'valign': 'vcenter', 'text_wrap': True, 'font_size': 10})
        ...
        title_format = workbook.add_format({'bold': True, 'font_size': 14, 'bg_color': '#f1f5f9', 'border': 1, 'align': 'center'})
        
        # ==================== RECONCILIATION SHEET ====================
        recon_df = merged_df[['MATCH_STATUS', 'MATCH_REASON', 'SUPPLIER_NAME_COMBINED', 'SUPPLIER_GSTIN_2B', 'SUPPLIER_GSTIN_PR', 'MY_GSTIN_2B', 'MY_GSTIN_PR', 'DOC_NUMBER_2B', 'DOC_NUMBER_PR', 'DOC_DATE_2B', 'DOC_DATE_PR', 'TOTAL_DOC_VALUE_2B', 'TOTAL_DOC_VALUE_PR', 'TAXABLE_VALUE_2B', 'TAXABLE_VALUE_PR', 'TAXABLE_DIFF', 'TOTAL_TAX_2B', 'TOTAL_TAX_PR', 'IGST_2B', 'IGST_PR', 'CGST_2B', 'CGST_PR', 'SGST_2B', 'SGST_PR', 'CESS_2B', 'CESS_PR', 'DOC_TYPE_2B', 'DOC_TYPE_PR', 'SECTION_NAME_2B', 'SECTION_NAME_PR', 'ITC_ELIGIBILITY']].copy()
        
        recon_df.columns = ['Match Status', 'Match Reason', 'Supplier Name', 'Supplier GSTIN (2B)', 'Supplier GSTIN (PR)', 'My GSTIN (2B)', 'My GSTIN (PR)', 'Document Number (2B)', 'Document Number (PR)', 'Document Date (2B)', 'Document Date (PR)', 'Total Value (2B)', 'Total Value (PR)', 'Taxable (2B)', 'Taxable (PR)', 'Taxable Diff', 'Total Tax (2B)', 'Total Tax (PR)', 'IGST (2B)', 'IGST (PR)', 'CGST (2B)', 'CGST (PR)', 'SGST (2B)', 'SGST (PR)', 'CESS (2B)', 'CESS (PR)', 'Doc Type (2B)', 'Doc Type (PR)', 'Section (2B)', 'Section (PR)', 'ITC Eligibility']
        ...
        if add_dropdown:
            doc_type_2b_col = doc_type_pr_col = None
            for idx, col in enumerate(recon_df.columns):
                if col == 'Doc Type (2B)': doc_type_2b_col = idx
                elif col == 'Doc Type (PR)': doc_type_pr_col = idx
            if doc_type_2b_col is not None:
                worksheet.data_validation(start_row + 1, doc_type_2b_col, start_row + len(recon_df), doc_type_2b_col, {'validate': 'list', 'source': ['INVOICE', 'CREDIT', 'DEBIT']})
            if doc_type_pr_col is not None:
                worksheet.data_validation(start_row + 1, doc_type_pr_col, start_row + len(recon_df), doc_type_pr_col, {'validate': 'list', 'source': ['INVOICE', 'CREDIT', 'DEBIT']})
        ...
        summary_ws.set_column('A:A', 35); summary_ws.set_column('B:B', 15); summary_ws.set_column('C:C', 45); summary_ws.set_column('D:D', 20); summary_ws.set_column('E:E', 20); summary_ws.set_column('F:F', 15)
...
if file_2b and file_pr:
    try:
        with st.spinner("🚀 Running Advanced Reconciliation Engine..."):
            ...
            merged_df, dup_pr_count, df_2b, df_pr, stats = process_reconciliation(file_2b.getvalue(), file_pr.getvalue(), tolerance, date_tolerance, include_reverse_charge, handle_cdn_negative, fuzzy_threshold, validate_gstin, strict_financial_year)
            
            st.session_state.processed_data = {'merged': merged_df, 'df_2b': df_2b, 'df_pr': df_pr, 'stats': stats}
            ...
            risky_claims = float(merged_df[merged_df['MATCH_STATUS'] == 'Missing in 2B']['TOTAL_TAX_PR'].sum())
            ...
            for idx, card in enumerate(status_cards):
                with cols[idx]:
                    st.markdown(f"""<div class="match-status-card {card['color']}"><div class="status-header"><span class="status-name">{card['icon']} {card['status']}</span><span class="status-count">{card['count']:,}</span></div><div class="status-details">{card['desc']}</div><div class="status-link">🔗 {card['link']}</div></div>""", unsafe_allow_html=True)
            
            # ==================== TOP 10 PARTIES ====================
            st.markdown("""<div class="section-card animate-fade-in"><h3><span class="icon">🏆</span> Top 10 Suppliers Analysis</h3></div>""", unsafe_allow_html=True)
            ...
            with col_top1:
                ...
                for idx, row in top_10_2b.iterrows():
                    st.markdown(f"""<div class="party-card"><div class="party-info"><div class="party-name">#{idx+1} {row['Supplier'][:40]}{'...' if len(row['Supplier'])>40 else ''}</div></div><div class="party-stats"><div class="party-value">{format_currency(row['Taxable Value'])}</div><div class="party-label">Taxable</div></div></div>""", unsafe_allow_html=True)
            with col_top2:
            ...
            with tab3:
                ...
                if status_filter: filtered = filtered[filtered['MATCH_STATUS'].isin(status_filter)]
                if search: filtered = filtered[filtered['SUPPLIER_NAME_COMBINED'].str.contains(search, case=False, na=False)]
                if min_val > 0: filtered = filtered[(filtered['TAXABLE_VALUE_2B'].abs() >= min_val) | (filtered['TAXABLE_VALUE_PR'].abs() >= min_val)]
                display_cols = ['MATCH_STATUS', 'SUPPLIER_NAME_COMBINED', 'DOC_TYPE_2B', 'DOC_NUMBER_2B', 'DOC_NUMBER_PR', 'TAXABLE_VALUE_2B', 'TAXABLE_VALUE_PR', 'TOTAL_TAX_2B', 'TOTAL_TAX_PR', 'TAX_DIFF', 'ITC_ELIGIBILITY']
                def apply_status_styling(val): return get_status_css_class(val)
                styled_df = filtered[display_cols].head(100).style.format({'TAXABLE_VALUE_2B': '₹{:.2f}', 'TAXABLE_VALUE_PR': '₹{:.2f}', 'TOTAL_TAX_2B': '₹{:.2f}', 'TOTAL_TAX_PR': '₹{:.2f}', 'TAX_DIFF': '₹{:.2f}'}).map(apply_status_styling, subset=['MATCH_STATUS'])
                st.dataframe(styled_df, use_container_width=True, hide_index=True)
            
            # ==================== AI INSIGHTS ====================
            st.markdown("""<div class="section-card animate-fade-in"><h3><span class="icon">🧠</span> AI-Powered Financial Insights</h3></div>""", unsafe_allow_html=True)
            insights = []
            if dup_pr_count > 0: insights.append({'type': 'warning', 'icon': '⚠️', 'title': 'Data Quality Alert', 'message': f"Found **{dup_pr_count} duplicate entries** in Purchase Register. Review for data integrity."})
            if missing_pr > 0: insights.append({'type': 'success', 'icon': '💡', 'title': 'Cash Flow Opportunity', 'message': f"**{format_currency(unclaimed_itc)}** in ITC available in GSTR-2B but not claimed. Consider claiming."})
            ...
            with col_export2:
                ...
                if export_format in ["CSV (.csv)", "Both"]:
                    ...
                    st.download_button(label="📄 Download CSV", data=csv_output.getvalue(), file_name=f"GST_Recon_Data_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv", use_container_width=True, key="btn_download_csv")
        ...
        with st.expander("🔧 Technical Details"):
            st.exception(e)
            st.info("""💡 **Troubleshooting:** - Ensure files follow sample template format - DOC_TYPE: INVOICE, CREDIT, or DEBIT - Credit Notes: Use negative values - GSTIN: Valid 15-character format - Dates: DD-MM-YYYY, YYYY-MM-DD, or DD/MM/YYYY""")
...
if 'load_sample' not in st.session_state: st.session_state.load_sample = False
if 'file_2b_hash' not in st.session_state: st.session_state.file_2b_hash = None
if 'file_pr_hash' not in st.session_state: st.session_state.file_pr_hash = None
...
if 'page_load_start' not in st.session_state: st.session_state.page_load_start = time.time()
else:
    load_time = time.time() - st.session_state.page_load_start
    if load_time > 5: logger.warning(f"Page load time: {load_time:.2f}s")
